...
import type { Language, ClientPhase, MatchInfo, GameData } from '../types'
...
export default function ClientPage() {
  ...
  const handleMessage = useCallback((body: string) => {
    try {
      const json = JSON.parse(body) as MatchInfo | GameData
      if ('taskType' in json) {
        const match = json as MatchInfo
        setValidatorLang(t(lang, `lang_${match.validatorLanguage ?? 'en'}` as Parameters<typeof t>[1]))
  ...
  // Global mouse tracking — canvasRef.current를 클로저 캡처 없이 매번 직접 읽음
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      let cx = -1
      let cy = -1
      const canvas = canvasRef.current   // 매번 ref에서 직접 읽기
  ...
  // Canvas 이벤트 핸들러 — React 합성 이벤트로 JSX에서 직접 부착
  ...
  const sendRps = (choice: string) => {
    send('/app/game', { type: 'RPS', payload: choice })
  ...
  const sendTyping = (text: string) => {
    send('/app/game', { type: 'CHAT_TYPING', payload: text })
  }
