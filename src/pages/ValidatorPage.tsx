...
import type { Language, ValidatorPhase, MatchRow, MouseData, SessionBroadcast, TaskType } from '../types'
...
export default function ValidatorPage() {
  ...
  const currentTaskRef = useRef<TaskType>('MOUSE_TRACKING')
  ...
  const handleMatchFound = useCallback((match: MatchRow) => {
    setClientLang(t(lang, `lang_${match.client_language ?? 'en'}` as Parameters<typeof t>[1]))
    setPhase('session')
  ...
  const drawMouseOnMirror = useCallback((pos: MouseData) => {
    ...
    if (currentTaskRef.current === 'DRAWING' && pos.isDown && pos.cx >= 0 && pos.cx <= 1 && pos.cy >= 0 && pos.cy <= 1) {
      const canvas = mirrorCanvasRef.current
      if (!canvas) return
      const ctx = canvas.getContext('2d')
      if (!ctx) return
  ...
  const handleMessage = useCallback((event: SessionBroadcast) => {
    if (event.event === 'mouse') {
      drawMouseOnMirror(event.payload)
  ...
  }, [lang, drawMouseOnMirror])

  const { sendGame, sendDecision } = useSupabase({
    uuid,
    language: lang,
    role: 'validator',
  ...
  const changeTask = (taskName: string) => {
    ...
    sendGame({ type: 'TASK_CHANGE', payload })
    applyTask(payload)
  }

  const sendRpsChallenge = (move: string) => {
    sendGame({ type: 'RPS_CHALLENGE', payload: move })
